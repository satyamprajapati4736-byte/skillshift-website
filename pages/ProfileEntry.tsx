...
const ProfileEntry: React.FC<ProfileEntryProps> = ({ onSuccess, onBack, redirectMessage }) => {
  ...
  const handlePhoneAction = async (e: React.FormEvent) => {
    ...
    try {
      if (isForgotPassword) {
        if (!phone || phone.length < 10) throw new Error("10-digit phone number daalein.");
    ...
    return (
      <div className="flex flex-col gap-10 animate-fadeIn py-8 px-2">
        <div className="text-center flex flex-col gap-3">
          ...
          <h2 className="text-4xl font-bold font-heading text-white tracking-tight">Login to Path</h2>
          <p className="text-slate-500 text-sm">Apna rasta chuno start karne ke liye.</p>
        </div>
        ...
        <div className="flex flex-col gap-4">
          <button 
            ...
            disabled={loading}
            className="w-full py-5 bg-white text-slate-900 rounded-2xl font-bold flex items-center justify-center gap-3 shadow-xl active:scale-95 transition-all disabled:opacity-50"
          >
          ...
          <div className="flex items-center gap-4 my-2">
            ...
            <span className="text-[10px] text-slate-600 font-bold uppercase tracking-widest">or use phone</span>
            <div className="h-px flex-1 bg-slate-800"></div>
          </div>
  ...
  return (
    <div className="flex flex-col gap-8 animate-fadeIn py-6 px-1">
      <div className="text-center flex flex-col gap-2">
        ...
        <p className="text-slate-500 text-sm">Phone se login karein.</p>
      </div>
      ...
      <form onSubmit={handlePhoneAction} className="flex flex-col gap-5">
        ...
        >
          {loading ? <ProgressSpinner /> : <span>{isForgotPassword ? "Send Link" : isLogin ? "Login" : "Sign Up"}</span>}
        </button>
