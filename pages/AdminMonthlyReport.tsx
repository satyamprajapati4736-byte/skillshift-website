...
const AdminMonthlyReport: React.FC<AdminMonthlyReportProps> = ({ user }) => {
  ...
  useEffect(() => {
    ...
    if (lastSent) {
      ...
      const now = new Date();
      // Check if sent in the same month
  ...
  const reportData = useMemo(() => {
    ...
    const monthlyUsers = allUsers.filter(u => new Date(u.created_at) >= startOfMonth);
    const monthlyRoadmaps = allRoadmaps.filter(r => new Date(r.created_at) >= startOfMonth);

    // Roadmap breakdown
    const roadmapCounts: Record<string, number> = {};
    monthlyRoadmaps.forEach(rm => {
...
• Total users this month: ${monthlyUsers.length}
• Total roadmaps generated: ${monthlyRoadmaps.length}

Top roadmaps of the month:
${sortedRoadmaps.length > 0 ? sortedRoadmaps.map(([name, count], i) => `${i + 1}) ${name} – ${count} users`).join('\n') : "No data yet."}

User trend:
• New users joined: ${monthlyUsers.length}
• Most popular roadmap: ${topRoadmap}

Overall summary:
“Is month platform growth stable rahi aur users ne
${topRoadmap} ko sabse zyada prefer kiya.”
--------------------------------`;
  ...
  return (
    <div className="flex flex-col gap-6 animate-fadeIn py-8 px-2">
      <div className="flex flex-col gap-2 mb-4">
        <h2 className="text-2xl font-bold font-heading text-white">Monthly Report</h2>
        <p className="text-slate-500 text-sm">Review and share the performance of the entire month.</p>
      </div>

      {reportSent && (
        <div className="bg-blue-500/10 border border-blue-500/20 p-4 rounded-2xl flex items-center gap-3 text-blue-400 text-xs font-medium">
          <span>📅</span>
          Report for {reportData.monthName} has already been generated/sent.
        </div>
      )}
      ...
      <div className="mt-8 text-[10px] text-slate-600 italic text-center leading-relaxed">
        * Monthly reports are generated based on active data from the current calendar month.<br/>
        Share responsibly with stakeholders.
      </div>
    </div>
  );
};
